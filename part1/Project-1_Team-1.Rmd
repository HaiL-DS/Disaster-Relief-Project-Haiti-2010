...
# correlations between color layers
RGB_only_train <- train_DRH %>% 
  dplyr::select(Red, Green, Blue)

T <- cor(RGB_only_train, )
corrplot(T, method='number', type='lower', col = COL2('RdBu', 5))

RGB_only_holdout <- holdout_DRH %>% 
  dplyr::select(Red, Green, Blue)

H = cor(RGB_only_holdout)
corrplot(H, method='number', type='lower', col = COL2('RdBu', 100))
...
# code for figure 6
# training
b1 <- ggplot(train_data, aes(x=Red, y=Blue, color=BT))+
  geom_point(size=1, alpha=0.25)+
  labs(title="Red vs Blue with Training Data")

b2 <- ggplot(holdout_data, aes(x=Red, y=Blue, color=BT))+
  geom_point(size=0.1, alpha=0.25)+
  labs(title="Red vs Blue with Holdout Data", caption = "Figure 1.2: Scatterplots of red and blue layers as they relate to the detection of blue tarp")
